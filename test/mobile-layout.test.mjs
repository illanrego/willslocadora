...
test('2D shelf and desktop inspection logos retry without hiding the title fallback', () => {
  const app = read('public/app.js');
  ...
  assert.match(viewer, /const ASSET_LOAD_ATTEMPTS = 6/);
  assert.match(viewer, /function attemptAssetLoad\(name, url\)/);
  assert.match(viewer, /assetRetryTimers\[name\] = window\.setTimeout/);
  assert.match(viewer, /dispose\(\) \{\s*disposed = true;\s*Object\.keys\(assetRetryTimers\)\.forEach\(clearAssetRetry\)/);
});
...
test('long browsing sessions keep hidden shelves and rendering work bounded', () => {
  const app = read('public/app.js');
  const immersive = read('public/immersive-shelf.mjs');
  const balcony = read('public/balcony.mjs');
  ...
  assert.match(immersive, /setActive\(active\) \{[\s\S]*cancelAnimationFrame\(frame\);[\s\S]*frame = 0;/);
  assert.match(app, /const dialogActivityObserver = new MutationObserver\(syncSceneActivity\)/);
  assert.match(app, /immersiveShelf\?\.setActive\?\.\(!dialogOpen\)/);
  assert.match(app, /balcony\?\.setActive\?\.\(!dialogOpen\)/);
  assert.match(balcony, /function setActive\(active\) \{[\s\S]*cancelAnimationFrame\(frame\);[\s\S]*frame = 0;/);
